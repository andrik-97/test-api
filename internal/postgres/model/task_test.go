...
func testTasksSoftDelete(t *testing.T) {
	t.Parallel()
...
func testTasksQuerySoftDeleteAll(t *testing.T) {
	t.Parallel()
	...
	if rowsAff, err := Tasks().DeleteAll(ctx, tx, false); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testTasksSliceSoftDeleteAll(t *testing.T) {
	t.Parallel()
...
func testTasksQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := Tasks().DeleteAll(ctx, tx, true); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
var (
	taskDBTypes = map[string]string{`ID`: `uuid`, `Title`: `text`, `Completed`: `boolean`, `CompletedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)

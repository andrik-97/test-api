...
var TaskWhere = struct {
	...
	CompletedAt: whereHelpernull_Time{field: "\"task\".\"completed_at\""},
...
var (
	taskAllColumns            = []string{"id", "title", "completed", "completed_at", "created_at", "updated_at", "deleted_at"}
	taskColumnsWithoutDefault = []string{"id", "title", "completed_at", "created_at", "updated_at", "deleted_at"}
...
func Tasks(mods ...qm.QueryMod) taskQuery {
	mods = append(mods, qm.From("\"task\""), qmhelper.WhereIsNull("\"task\".\"deleted_at\""))
...
func FindTask(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Task, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"task\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *Task) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"task\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "model: unable to upsert task")
	}
...
func (o *Task) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"task\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func (o TaskSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"task\" SET %s WHERE "+
...
func TaskExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"task\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
